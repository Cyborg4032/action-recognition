...
1. **Keypoint Extraction** — MediaPipe Holistic detects face, pose, left-hand, and right-hand landmarks from each video frame.
2. **Data Collection** — For each action/sign, 30 video sequences of 30 frames each are captured and saved as `.npy` arrays in `MP_Data/`.
3. **Model Training** — A stacked LSTM network is trained on the sequences to learn temporal patterns in gestures.
4. **Real-Time Inference** — The trained model (`action.h5`) is loaded for live webcam detection with visual probability overlays.
...
- [Technical Documentation](./Sign_Language_Detection_Technical_Documentation.docx) — detailed system design and methodology
- [Presentation Slides](./Sign_Language_Detection_Presentation.pptx) — project overview and results
...
This project is open source. See [LICENSE](./LICENSE) for details (if applicable).
